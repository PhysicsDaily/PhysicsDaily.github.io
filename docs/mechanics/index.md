---
hide:
  - navigation
---

<div class="topic-hero">
  <div class="topic-hero-text">
    <p class="topic-hero-kicker">Part 1</p>
    <h1>Mechanics</h1>
    <p class="topic-hero-subtitle">Mechanics is the foundation of physics, dealing with motion, forces, energy, and the interactions between objects.</p>
    <a class="topic-hero-button" href="intro-to-mechanics.md">Intro to Mechanics</a>
  </div>
</div>

---

## Topics Covered

<div class="grid cards clickable-cards" markdown>

-   **Chapter 0**

    ---

    [**Units and Physical Quantities**](units-physical-quantities.md)

    Foundations of measurement and dimensional analysis.

-   **Chapter 1**

    ---

    [**Vectors**](vectors.md)

    Vector operations, components, and applications.

-   **Chapter 2**

    ---

    [**Motion Along a Straight Line**](motion-along-straight-line.md)

    Kinematics in one dimension.

-   **Chapter 3**

    ---

    [**Motion in Two or Three Dimensions**](motion-2d-3d.md)

    Projectile and circular motion.

-   **Chapter 4**

    ---

    [**Newton's Laws of Motion**](newtons-laws.md)

    Force, mass, and the three laws of motion.

-   **Chapter 5**

    ---

    [**Applying Newton's Laws**](applying-newtons-laws.md)

    Friction, drag, and circular dynamics.

-   **Chapter 6**

    ---

    [**Work and Kinetic Energy**](work-kinetic-energy.md)

    Work-energy theorem and power.

-   **Chapter 7**

    ---

    [**Potential Energy and Energy Conservation**](potential-energy-conservation.md)

    Conservative forces and energy conservation.

-   **Chapter 8**

    ---

    [**Momentum, Impulse, and Collisions**](momentum-impulse-collisions.md)

    Impulse, momentum, and collisions.

-   **Chapter 9**

    ---

    [**Rotation of Rigid Bodies**](rotation-rigid-bodies.md)

    Angular kinematics and energy.

-   **Chapter 10**

    ---

    [**Dynamics of Rotational Motion**](dynamics-rotational-motion.md)

    Torque and angular momentum.

-   **Chapter 11**

    ---

    [**Equilibrium and Elasticity**](equilibrium-elasticity.md)

    Static equilibrium and stress/strain.

-   **Chapter 12**

    ---

    [**Fluid Mechanics**](fluid-mechanics.md)

    Density, pressure, and buoyancy.

-   **Chapter 13**

    ---

    [**Gravitation**](gravitation.md)

    Newton's law of gravitation and orbits.

-   **Chapter 14**

    ---

    [**Periodic Motion**](periodic-motion.md)

    Oscillations and simple harmonic motion.

</div>

---
